...
EXTRACTED_JSON = BASE_DIR / "extracted_text_v6.json"

BODY_SIZE = 12.0
CHAPTER_SIZE = 16.0
SECTION_SIZE = 15.0
SUBSECTION_SIZE = 14.0
SMALL_SIZE = 10.8
...
def extract_page(page_num, doc):
    ...
    for line in lines:
        ...
        if y_bot > page_h - 70 and is_page_header_footer(t, page_num):
            continue
        # strip inline page header prefix from any line, but only for small text
        # (page headers are ~8pt; real section/chapter headings are 14pt+)
        size = avg_size(line)
        cleaned = strip_inline_page_header(t) if size < SECTION_SIZE - 1 else t
        if cleaned != t:
...
def normalize_body_text(page_data_list):
    ...
    def detect_heading_line(t, size, bold):
        t = t.strip()
        # English title (abstract chapter)
        if t.startswith("Research on a Clue Data-Aware SQL Generation Method"):
            return t, 1

        # Special unnumbered chapters / front matter
        if t in ("摘要", "ABSTRACT", "参考文献", "致谢", "攻读学位期间取得的研究成果"):
            return t, 1

        # Chapter headings: "1 绪论" or "1绪论" or "第1章 绪论" or "第一章 绪论"
        m = re.match(r"^第([1-9一二三四五六])章\s*(.+)$", t)
        if m and size >= CHAPTER_SIZE - 1:
            return f"{m.group(2).strip()}", 1
        m = re.match(r"^第([1-9])章\s*(.+)$", t)
        if m and size >= CHAPTER_SIZE - 1:
            return f"{m.group(2).strip()}", 1
        m = re.match(r"^([1-9])\s*([一-龥A-Z].{1,30})$", t)
        if m and size >= CHAPTER_SIZE - 1:
            return f"{m.group(1)} {m.group(2).strip()}", 1

        # Section headings: "1.1 选题背景与研究意义" or "1.1选题背景与研究意义"
        m = re.match(r"^(\d{1,2}\.\d{1,2})\s*([一-龥A-Z].+)$", t)
        if m and size >= SECTION_SIZE - 1:
            return f"{m.group(1)} {m.group(2).strip()}", 2

        # Subsection headings: "1.2.1 基于提示的大模型推理" or "1.2.1基于提示的大模型推理"
        m = re.match(r"^(\d{1,2}\.\d{1,2}\.\d{1,2})\s*([一-龥A-Z].+)$", t)
        if m and size >= SUBSECTION_SIZE - 1:
            return f"{m.group(1)} {m.group(2).strip()}", 3
...
def process_pdf():
    ...
    for p in pages:
        if prev_type in ("abstract_cn", "abstract_en") and p["type"] == "body":
            ...
            if not re.search(
                r"^(摘要|ABSTRACT|第[1-9一二三四五六]章|\d+\s*[一-龥A-Z]|\d+\.\d+\s*[一-龥A-Z])",
                first_lines_text,
            ):
                p["type"] = prev_type
        prev_type = p["type"]
...
def balance_braces(text):
    """Add missing { or } braces so LaTeX can compile fragmented formulas."""
    depth = 0
    for ch in text:
        if ch == "{":
            depth += 1
...
def unicode_to_latex_math(text):
    mappings = {
        ...
        "𝜅": r"\kappa", "𝜉": r"\xi", "𝜓": r"\psi",
        "𝜙": r"\phi", "𝜖": r"\varepsilon", "𝜀": r"\epsilon",
        "̂": "",
        "⎧": "{", "⎨": "{", "⎩": "{", "Γ": r"\Gamma",
        ...
        "∧": r"\land", "∨": r"\lor", "¬": r"\neg", "∣": r"\mid",
        "∥": r"\parallel", "⋀": r"\bigwedge", "⊙": r"\odot", "∼": r"\sim",
        "→": r"\rightarrow",
        ...
        "𝒢": r"\mathcal{G}", "ℋ": r"\mathcal{H}", "𝒤": r"\mathcal{I}",
    ...
    # Commands that may need a trailing space when followed by a letter.
    simple_cmds = [
        r"alpha", r"beta", r"gamma", r"Gamma", r"delta", r"Delta", r"lambda", r"theta",
        r"Theta", r"sigma", r"Sigma", r"mu", r"nu", r"rho", r"tau", r"varphi", r"phi", r"Phi",
        r"omega", r"zeta", r"eta", r"kappa", r"xi", r"psi", r"Psi", r"pi", r"epsilon", r"varepsilon",
        r"in", r"mid", r"sum", r"prod", r"int", r"max", r"min",
        r"sqrt", r"cdot", r"times", r"div", r"pm", r"leq", r"geq", r"neq",
        r"approx", r"cup", r"cap", r"subset", r"subseteq", r"supset", r"supseteq",
        r"emptyset", r"forall", r"exists", r"land", r"lor", r"neg", r"rightarrow",
        r"leftarrow", r"Rightarrow", r"Leftarrow", r"leftrightarrow", r"Leftrightarrow",
        r"uparrow", r"downarrow", r"partial", r"nabla", r"infty", r"sim", r"langle", r"rangle",
        r"ldots", r"cdots", r"vdots", r"ddots", r"parallel", r"bigwedge", r"odot",
    ]
    cmd_pattern = r"\\(?:" + "|".join(simple_cmds) + r")"
    text = re.sub(rf"({cmd_pattern})([a-zA-Z])", r"\1 \2", text)
    ...
    # Heuristic: plain digits immediately after Greek / single commands that
    # were meant as superscripts in the PDF (e.g. \gamma2 -> \gamma^2).
    text = re.sub(rf"({cmd_pattern})([0-9])", r"\1^\2", text)

    # Replace bare \sqrt not followed by argument with a placeholder.
...
def generate_markdown(blocks, pages):
    ...
    for ptype in ("references", "acknowledgement", "publications"):
        ...
        matched_pages = [p for p in pages if p["type"] == ptype]
    ...
    md = "\n".join(out)
    # Merge the English title that was split across two lines by the PDF extractor.
    md = md.replace(
        "# Research on a Clue Data-Aware SQL Generation Method Based\n\non Multi-Agent Collaboration",
...
def normalize_math_chars(text):
    """Convert Unicode mathematical italic letters to ASCII in normal text."""
    italic_lower = "𝑎𝑏𝑐𝑑𝑒𝑓𝑔ℎ𝑖𝑗𝑘𝑙𝑚𝑛𝑜𝑝𝑞𝑟𝑠𝑡𝑢𝑣𝑤𝑥𝑦𝑧"
    ascii_lower = "abcdefghijklmnopqrstuvwxyz"
    italic_upper = "𝐴𝐵𝐶𝐷𝐸𝐹𝐺𝐻𝐼𝐽𝐾𝐿𝑀𝑁𝑂𝑃𝑄𝑅𝑆𝑇𝑈𝑉𝑊𝑋𝑌𝑍"
    ascii_upper = "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
    for ch, asc in zip(italic_lower, ascii_lower):
        text = text.replace(ch, asc)
    for ch, asc in zip(italic_upper, ascii_upper):
        text = text.replace(ch, asc)
    return text
...
def escape_latex(text):
    ...
    # First escape standard LaTeX special characters.
    chars = {
        "&": r"\&", "%": r"\%", "$": r"\$", "#": r"\#",
        "_": r"\_", "{": r"\{", "}": r"\}",
        "~": r"\textasciitilde{}",
    ...
    # Then replace inline math symbols with their LaTeX commands (unescaped $).
    math_text_map = {
        "⋅": r"$\cdot$", "×": r"$\times$", "÷": r"$\div$", "±": r"$\pm$",
        "≤": r"$\leq$", "≥": r"$\geq$", "≠": r"$\neq$", "≈": r"$\approx$",
        "∈": r"$\in$", "∉": r"$\notin$", "∪": r"$\cup$", "∩": r"$\cap$",
        ...
        "↔": r"$\leftrightarrow$", "⇔": r"$\Leftrightarrow$", "↑": r"$\uparrow$", "↓": r"$\downarrow$",
        "∑": r"$\sum$", "∏": r"$\prod$", "∫": r"$\int$", "∂": r"$\partial$",
        "∇": r"$\nabla$", "√": r"$\sqrt{?}$", "∞": r"$\infty$",
        "α": r"$\alpha$", "β": r"$\beta$", "γ": r"$\gamma$", "δ": r"$\delta$",
        "λ": r"$\lambda$", "θ": r"$\theta$", "σ": r"$\sigma$", "μ": r"$\mu$",
        "ν": r"$\nu$", "π": r"$\pi$", "ρ": r"$\rho$", "τ": r"$\tau$",
        "φ": r"$\varphi$", "ω": r"$\omega$", "ζ": r"$\zeta$", "η": r"$\eta$",
        "κ": r"$\kappa$", "ξ": r"$\xi$", "ψ": r"$\psi$",
        "Γ": r"$\Gamma$", "Δ": r"$\Delta$", "Θ": r"$\Theta$", "Λ": r"$\Lambda$",
        "Σ": r"$\Sigma$", "Φ": r"$\Phi$", "Ψ": r"$\Psi$", "Ω": r"$\Omega$",
        "⟨": r"$\langle$", "⟩": r"$\rangle$",
        # Mathematical italic Greek letters that may appear in normal text
        "𝛼": r"$\alpha$", "𝛽": r"$\beta$", "𝛾": r"$\gamma$", "𝛿": r"$\delta$",
        "𝜆": r"$\lambda$", "𝜃": r"$\theta$", "𝜎": r"$\sigma$", "𝜇": r"$\mu$",
        "𝜈": r"$\nu$", "𝜋": r"$\pi$", "𝜌": r"$\rho$", "𝜏": r"$\tau$",
        "𝜑": r"$\varphi$", "𝜔": r"$\omega$", "𝜁": r"$\zeta$", "𝜂": r"$\eta$",
        "𝜅": r"$\kappa$", "𝜉": r"$\xi$", "𝜓": r"$\psi$", "𝜀": r"$\epsilon$",
        "𝜖": r"$\varepsilon$", "𝜙": r"$\phi$",
        # Other math symbols
        "′": r"$'$", "∣": r"$|$", "▷": r"$\triangleright$", "△": r"$\triangle$",
        "∼": r"$\sim$", "ℒ": r"$\mathcal{L}$", "ℛ": r"$\mathcal{R}$",
        "𝒜": r"$\mathcal{A}$", "𝒟": r"$\mathcal{D}$", "𝔼": r"$\mathbb{E}$",
        "∥": r"$\parallel$", "⋀": r"$\bigwedge$", "⊙": r"$\odot$",
    }
    for ch, rep in math_text_map.items():
        text = text.replace(ch, rep)
    return text
    ...
    while i < len(lines):
        ...
        if not stripped:
            if in_code:
                ...
                close_lists()
                # blank line separates paragraphs in LaTeX
        ...
        if stripped.startswith("# "):
            ...
            # Replace literal \n with a space inside formulas
            formula = formula.replace("\\n", " ")
            # Balance braces to avoid compilation errors from fragmented equations
            formula = balance_braces(formula)
