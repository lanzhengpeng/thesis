...
# -*- coding: utf-8 -*-
"""
Convert thesis PDF to clean Markdown and LaTeX (v4).
Uses PyMuPDF layout info to identify headings, paragraphs, equations, etc.
"""
...
EXTRACTED_JSON = BASE_DIR / "extracted_text_v4.json"
...
def classify_page_type(text, page_num):
    s = text.strip()
    if page_num <= 3:
        return "cover"
    if "目录" in s[:10] or re.search(r"\.\.\.+\s*[IVXLC]+\s*$", s, re.M):
        return "toc"
    if "学位论文原创性声明" in s or "学位论文独创性声明" in s or "学位论文使用授权声明" in s:
        return "statement"
    if s.startswith("摘要") or s.startswith("摘 要"):
        return "abstract_cn"
    if s.startswith("ABSTRACT") or s.startswith("Research on a Clue"):
        return "abstract_en"
...
def extract_page(page_num, doc):
    ...
    # remove header/footer lines by position and content
    ...
    for line in lines:
        ...
        y_bot = line[0]["bbox"][3]
        t = line_text(line)
        if y_top < 60 and is_page_header_footer(t, page_num):
            continue
        if y_bot > page_h - 60 and is_page_header_footer(t, page_num):
            continue
        filtered.append(line)

    text = "\n".join(line_text(line) for line in filtered)
    ptype = classify_page_type(text, page_num)
...
def normalize_body_text(page_data_list):
    ...
    for page_data in page_data_list:
        for line in page_data["lines"]:
            ...
            # if font is small and no Chinese, likely figure text -> caption/figure text
            if size <= SMALL_SIZE and not re.search(r"[一-鿿]", t) and len(t) < 80:
                # keep with paragraph context, but flush first
                flush_para()
...
def process_pdf():
    ...
    body_pages = [p for p in pages if p["type"] == "body"]
...
def generate_markdown(blocks, pages):
    ...
    # Add references, acknowledgement, publications from raw pages
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# "):
            ...
            if in_code:
                ...
                in_code = True
        elif stripped.startswith("*") and stripped.endswith("*") and re.match(r"^[*](图|表|Fig|Table).*", stripped):
            close_lists()
            if in_code:
                body.append("\\\\end{lstlisting}")
                in_code = False
...
def main():
    ...
    print(f"Extracted {len(blocks)} blocks from {len([p for p in pages if p['type']=='body'])} body pages")
