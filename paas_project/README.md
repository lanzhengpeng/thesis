...
| `/admin/kernel/reload/{plugin_name}` | POST | 重新加载指定插件（刷新系统口容器；服务口需重启以刷新路由） |

> **注意**：
> 1. `/admin/kernel/reload` 仅刷新 **8000 系统口** 的内存容器，用于前端实时展示。
> 2. **8001 服务口** 的业务路由在进程启动时固定；新增/修改插件后，需重启服务口进程才能暴露新路由。
> 3. 这是 FastAPI 运行时无法注销已注册路由带来的限制，后续可通过动态路由层或应用级重启解决。
...
`/admin/kernel/reload` 只刷新 **8000 系统口** 的内存容器，用于前端画布实时展示。
**8001 服务口** 启动时会固定所有业务路由；FastAPI 不支持运行时注销已注册路由，因此新增/修改插件后，需要重启 8001 服务口进程才能暴露新路由。
...
| 端口 | 进程 | 用途 | 访问方 |
|------|------|------|--------|
| 8000 | System Server | 管理接口、作弊纸、健康检查 | 平台自身前端 / 管理员 |
| 8001 | Service Server | 业务 API（/api/*） | 外部调用方 / 客户 |

两者作为独立进程运行，互不干扰。系统口可以独立重启或做热重载，不影响服务口的业务流量。
